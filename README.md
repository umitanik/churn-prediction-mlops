...
to run. This is intentional: a service that cannot make predictions should fail loudly
...
The API needs a reachable PostgreSQL instance; point it at one with
`DATABASE_URL=postgresql://admin:654321@localhost:5432/churn_db`.
...
|---|---|---|
| `GET` | `/health` | Model and database status. Returns `503` when the service is degraded. |
